...
phos.diffex.latevsearly <- limmaTwoFactorDEAnalysis(phos.mat, treated.early$Sample, treated.late$Sample)

############################ Saving tables for further use ##################################
# write.table(phos.diffex.treated, "Treated - Healthy.txt", sep = "\t")
# 
# write.table(phos.diffex.pre, "Pre-Treatment - Healthy.txt", sep = "\t")
# 
# write.table(phos.diffex.early, "Early - Healthy.txt", sep = "\t")
# 
# write.table(phos.diffex.late, "Late - Healthy.txt", sep = "\t")
# 
# write.table(phos.diffex.earlyvspre, "Early - Pre-Treatment.txt", sep = "\t")
# 
# write.table(phos.diffex.latevsearly, "Late - Early.txt", sep = "\t")
...
ksea.earlyvspre <- phos.diffex.latevsearly %>%
  tibble::rownames_to_column('site') %>%
  left_join(gene.to.site) %>%
  dplyr::select(Gene,Peptide,residue,value='logFC',p_adj='adj.P.Val') %>%
  computeKSEA(prefix = "Early-vs-late", ksea_FDR = 0.05)
...
```{r Late vs Early}
show <- phos.diffex.latevsearly %>%
  select(-featureID, -t, -B) %>% head(10)
kbl(show) %>%
  kable_paper("hover", full_width = F)
```
